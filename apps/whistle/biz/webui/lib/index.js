...
function sendV2Index(res) {
  common.getStat(v2IndexFile, function(err, stat) {
    if (!err && stat && stat.isFile()) {
      return res.sendFile(v2IndexFile);
    }
    common.getStat(v2LegacyIndexFile, function(legacyErr, legacyStat) {
      if (legacyErr || !legacyStat || !legacyStat.isFile()) {
        return res.status(404).end('Not Found');
      }
      res.sendFile(v2LegacyIndexFile);
    });
  });
}

function proxyV2Req(req, res, next) {
  if (!enableV2Proxy || !util || req.path.indexOf('/v2') !== 0) {
    return next();
  }
  req.url = req.originalUrl || req.url;
  req.headers.host = '127.0.0.1:' + v2Port;
  util.transformReq(req, res, v2Port);
}
